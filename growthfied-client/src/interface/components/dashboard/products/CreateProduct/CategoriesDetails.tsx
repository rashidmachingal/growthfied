...
export default function CategoriesDetails({ setProductData, productData }: CategoriesDetailsProps) {
      ...
      const handleCategoriesChange = (id: string) => {
        if(productData.categories.includes(id)){
          const categoryRemovedData = {
            ...productData,
            categories: productData.categories.filter(category => category !== id)
          }
          setProductData(categoryRemovedData)
        }else{
          setProductData(prevData => ({
            ...prevData,
    ...
    <Div className="w-full bg-white rounded-[4px] px-[12px] pb-[2rem] pt-[10px] mt-[23px]">
      ...
      <Div className="mt-[9px] flex flex-col gap-2 h-[8rem] overflow-y-auto style-scroll">
        ...
        {categories?.map((data: any) => {
          let checked = productData.categories.some((id: string) => {
            return data._id === id;
          });
          return (
            <Div
